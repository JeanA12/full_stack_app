       ...
       stage('Build Frontend') {
        ...
        stage('Deploy') {
            steps {
                ...
                script {
                    sh 'docker run -d -p 1414:80 nom_image_frontend:latest' // Exemple pour le front-end
                    sh 'docker run -d -p 1212:8080 nom_image_backend:latest' // Exemple pour le back-end
                }
            }
        }
    }
